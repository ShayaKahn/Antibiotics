...
class HC:
    ...
    """
    def __init__(self, num_samples, pool_size, num_survived, mean, sigma, lower, higher, delta, final_time, max_step,
                 epsilon, threshold, min_skew, max_skew, interaction_strength, max_growth, min_growth,
                 method='RK45', measure='Jaccard'):
        """
        ...
        """
        if not (isinstance(num_samples, int) and isinstance(pool_size, int) and isinstance(num_survived, int)):
        ...
        self.A = self._set_interaction_matrix()
        self.steady_state = np.linalg.solve(self.A+np.diag(self.s), -self.r)
        self.perturbed_state = self._apply_GLV(self.y0, norm=True)
        self.new_perturbed_state = self._normalize_cohort(self._insert_total_pool())
        self.post_perturbed_state = self._apply_GLV(self.new_perturbed_state, norm=True)
    ...
    def _set_initial_conditions(self):
        ...
        survived_matrix = np.array([random.sample(range(0, self.pool_size),
    ...
    def _set_interaction_matrix(self):
        ...
        #random_numbers = np.random.normal(self.mean, self.sigma, size=(self.pool_size, self.pool_size))*1.8
