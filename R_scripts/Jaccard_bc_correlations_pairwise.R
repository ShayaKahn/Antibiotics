...
pairwise_jaccard_bray_curtis <- function(ABX_cohort, post_ABX_cohort) {
  ...
  for (i in 1:(n-1)) {
    for (j in (i+1):n) {
      ...
      union_ABX <- length(union(nonzero_i_ABX, nonzero_j_ABX))
      
      # Calculate Jaccard similarity
      similarity_ABX <- intersection_ABX / union_ABX
      
      # Append the similarity to the vector
      jaccard_ABX <- c(jaccard_ABX, similarity_ABX)
      
      concat <- c(post_ABX_cohort[i, ][-inersection_ind_ABX] / sum(post_ABX_cohort[i, ][-inersection_ind_ABX]),
                  post_ABX_cohort[j, ][-inersection_ind_ABX]/ sum(post_ABX_cohort[j, ][-inersection_ind_ABX]))
      m <- matrix(concat, nrow=2,
                  ncol=length(post_ABX_cohort[i, ][-inersection_ind_ABX]),
                  byrow = TRUE)
      
      # Calculate bray curtis
      bc <- as.numeric(vegdist(m, method = "bray"))
      
      # Append the similarity to the vector
      bc_post_ABX <- c(bc_post_ABX, bc)
    }
  }
  return(list("jaccard_ABX" = jaccard_ABX,
              "bc_post_ABX" = bc_post_ABX))
}
...
results_jaccard <- pairwise_jaccard_bray_curtis(recovery_ABX, recovery_post_ABX)

x_recovery_jaccard <- results_jaccard$"jaccard_ABX"
y_recovery_jaccard <- results_jaccard$"bc_post_ABX"
...
results_jaccard <- pairwise_jaccard_bray_curtis(gut_ABX, gut_post_ABX)

x_gut_jaccard <- results_jaccard$"jaccard_ABX"
y_gut_jaccard <- results_jaccard$"bc_post_ABX"
...
results_jaccard <- pairwise_jaccard_bray_curtis(spo_ABX, spo_post_ABX)

x_spo_jaccard <- results_jaccard$"jaccard_ABX"
y_spo_jaccard <- results_jaccard$"bc_post_ABX"
...
results_jaccard <- pairwise_jaccard_bray_curtis(sim_ABX, sim_post_ABX)

x_sim_jaccard <- results_jaccard$"jaccard_ABX"
y_sim_jaccard <- results_jaccard$"bc_post_ABX"
...
results_jaccard <- pairwise_jaccard_bray_curtis(sim_ABX, sim_post_ABX_shuffled)

x_sim_shuffled_jaccard <- results_jaccard$"jaccard_ABX"
y_sim_shuffled_jaccard <- results_jaccard$"bc_post_ABX"
